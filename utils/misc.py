...
# def add_batch(self, label_pred, label_true,TRE=False):
#     assert label_true.shape == label_pred.shape
#     self.hist += self._fast_hist(label_pred, label_true)
#     if TRE:
#         return self.hist

def evaluate(self,predictions, gts, num_classes):
    self.hist = np.zeros((num_classes, num_classes))

    for lp, lt in zip(predictions, gts):
        self.hist += _fast_hist(lp.flatten(), lt.flatten(), num_classes)
        #miou_curr_meter.update(hist)
    ...
    acc_cls = np.diag(self.hist) / np.sum(self.hist,axis=1)
    acc_cls = np.nanmean(acc_cls)
    #iu = np.diag(hist) / (hist.sum(axis=1) + hist.sum(axis=0) - np.diag(hist))
    iu = np.diag(self.hist) / (np.sum(self.hist,axis=1) + np.sum(self.hist,axis=0) - np.diag(self.hist))
    #print('iu:',iu )
