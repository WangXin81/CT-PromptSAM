...
# def get_random_points(pred_sem_seg, mask_value, foreground_points=500, background_points=250):
...
#     # else:
#     print('mask:', pred_sem_seg)
#     foreground_indices = np.argwhere(pred_sem_seg == mask_value)
#     print('foreground_indices：', foreground_indices)
#
#     background_indices = np.argwhere(pred_sem_seg != mask_value)
#     print('background_indices:', background_indices)
#
#     # Shuffle the indices to get random points
#     np.random.shuffle(foreground_indices)
#     np.random.shuffle(background_indices)
#
#     if (foreground_indices != 0) & (background_indices != 0):
...
#         # left clicks
#         try:
#             left_clicks = foreground_indices[
#                 np.random.choice(foreground_indices.shape[0], foreground_points, replace=False)]
#
#         except:
#             left_clicks = np.empty(0)
#
#         try:
#             right_clicks = background_indices[
#                 np.random.choice(background_indices.shape[0], background_points, replace=False)]
#         except:
...
#def get_random_points(mask, mask_value, foreground_points=125, background_points=63):
def get_random_points(mask, mask_value, foreground_points=125, background_points=63):
    ## road = 75, sidewalk/crosswalk = 29, background = 0
    # Find the indices (coordinates) of the foreground pixels
    ...
    # print('foreground_points:',foreground_points)
